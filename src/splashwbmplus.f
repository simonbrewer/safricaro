...
c  !      
c  ! Added estimate of land source runoff (in addition to saturation excess)
c  ! Using WBM definition (excess moisture goes to detention pool)
c  ! 
c  !----------------------------------------------------------------

c******************************************************************************c
        subroutine spin_up_sm( yr,lat,elv,whc,pr,tc,sf,
     >                         maet,mpet,mcn,mro,mlsr,msm,msnow,mdr,
     >                         daet,dpet,dcn,dro,dlsr,dsm,dsnow,dmelt,
     >                         ddr,sm)
        ...
        double precision snow   ! snowpack (mm) SB
        double precision dr     ! runoff detention storage (mm) SB
        ...
        double precision ddr(365)
        double precision dmelt(365)
        ...
        ro = 0.0
        lsr = 0.0
        dr = 0.0 ! initialize detention pool SB
...
c******************************************************************************c
     ...
     >                           mdr,ddr,dr)
        ...
        double precision melt  ! snow melt (mm) SB
        double precision dr    ! runoff detention pool (mm) SB
     ...
     *   / 31,28,31,30,31,30,31,31,30,31,30,31          /
     ...
     >                    aet,pet,sm,cn,ro,lsr,snow,melt,dr )
 ...
 20     continue
        ...
        ! Mean detention pool for month
        mdr(moy) = mdr(moy) / ndaymonth(moy)
...
c******************************************************************************c
     ...
     >                          aet,pet,sm,cn,ro,lsr,snow,melt,dr)
        ...
        double precision maxsnow  ! max snowpack (mm) SB
        double precision melt  ! daily melt (mm, from GUESS) SB
        double precision dr  ! detention pool (mm, from WBM) SB
        double precision drgamma  ! detention pool parameter WBM
        double precision drbeta  ! detention pool parameter WBM
        ...
        parameter(drgamma = 0.5) ! Proportion of runoff direct to river
                                 ! Wisser et al 2008 (mm d-1)
        parameter(drbeta = 0.0167) ! Outflow from detention pool (d-1)
        ...
        !print*,doy,tc,pr,snow,melt,aet,cn,sm
        ...
        ! Update soil detention pool
        dr = dr + ((1 - drgamma) * ro) - (drbeta * dr) 

        ! Calculate land surface runoff (Wisser et al 2008)
        lsr = (drgamma * ro) + (drbeta * dr) 

        ! Calculate land surface runoff (Miller et al 1994)
        !if (kWm > 0) then
        !        lsr1 = 0.5 * (pr + melt) * (sm / kWm)
        !        ! lsr2 = 0.5 * (pr + melt + sm - kWm)
        !        lsr2 = pr + melt + sm - kWm 
        !        lsr = max(lsr1,lsr2)
        !else
        !        lsr = pr + melt
